...
install_uvx_mcps() {
    ...
    local count=$(jq '.uvx | length' "$REGISTRY_FILE")

    for ((i=0; i<count; i++)); do
        local name=$(jq -r ".uvx[$i].name" "$REGISTRY_FILE")
        local command=$(jq -r ".uvx[$i].command" "$REGISTRY_FILE")
        local args=$(jq -r ".uvx[$i].args | join(\" \")" "$REGISTRY_FILE")
        local docker_image=$(jq -r ".uvx[$i].docker_image // empty" "$REGISTRY_FILE")
        ...
        # Build env args
        local env_args=""
        local env_count=$(jq ".uvx[$i].env | length" "$REGISTRY_FILE")
