...
configure_shell() {
  ...
  local shell_rc=""
  # shellcheck disable=SC2034  # shell_name used for logging in future
  local shell_name=""
...
start_infrastructure() {
  ...
  # shellcheck disable=SC2043  # Single container now, may expand later
  ...
  while [ $waited -lt $max_wait ]; do
    local qdrant_healthy
    qdrant_healthy=$(docker inspect --format='{{.State.Health.Status}}' codeagent-qdrant 2>/dev/null || echo "none")
