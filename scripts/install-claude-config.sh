...
validate_skill() {
  ...
  if command -v yq &>/dev/null; then
    ...
    local name
    name=$(echo "$frontmatter" | yq -r '.name' 2>/dev/null)
    local desc
    desc=$(echo "$frontmatter" | yq -r '.description' 2>/dev/null)
...
install_skills() {
  ...
  if [ -f "$REGISTRY_FILE" ]; then
    local skill_count
    skill_count=$(jq '.skills | length' "$REGISTRY_FILE" 2>/dev/null || echo "0")
  ...
  if [ "$use_registry" = "true" ]; then
    for ((i = 0; i < skill_count; i++)); do
      local skill_name
      skill_name=$(jq -r ".skills[$i].name" "$REGISTRY_FILE")
      local skill_source
      skill_source=$(jq -r ".skills[$i].source" "$REGISTRY_FILE")
...
install_commands() {
  ...
  if [ -f "$REGISTRY_FILE" ]; then
    local cmd_count
    cmd_count=$(jq '.commands | length' "$REGISTRY_FILE")

    for ((i = 0; i < cmd_count; i++)); do
      local cmd_name
      cmd_name=$(jq -r ".commands[$i].name" "$REGISTRY_FILE")
      local cmd_source
      cmd_source=$(jq -r ".commands[$i].source" "$REGISTRY_FILE")
...
install_hooks() {
  ...
  if [ -f "$REGISTRY_FILE" ]; then
    local hook_count
    hook_count=$(jq '.hooks | length' "$REGISTRY_FILE")

    for ((i = 0; i < hook_count; i++)); do
      local hook_name
      hook_name=$(jq -r ".hooks[$i].name" "$REGISTRY_FILE")
      local hook_source
      hook_source=$(jq -r ".hooks[$i].source" "$REGISTRY_FILE")
...
install_agents() {
  ...
  if [ "$use_registry" = "true" ]; then
    for ((i = 0; i < agent_count; i++)); do
      local agent_name
      agent_name=$(jq -r ".agents[$i].name" "$REGISTRY_FILE")
      local agent_source
      agent_source=$(jq -r ".agents[$i].source" "$REGISTRY_FILE")
